 ...
 * 
 * La seguente classe si occupa di gestire i metodi della periferica OpticalBarrier.
...
class OpticalBarrier {
     ...
     * @returns {object} {topic: "OpticalBarrier/telemetry/identity"} 
     */
    identity() {
        const topic = 'OpticalBarrier/telemetry/identity'  
        var message = { topic : topic }
        if(process.env.DEBUG == 1 && process.env.DEBUG_PERIPHERALS == 1) {
            console.log('@@@@@@ OpticalBarrier > identity > message ', message)
        }
        return message
    }
     ...
     * @returns {object} {topic: "OpticalBarrier/telemetry/status"} 
     */
    status() {
        const topic = 'OpticalBarrier/telemetry/status'  
        var message = { topic : topic }
        if(process.env.DEBUG == 1 && process.env.DEBUG_PERIPHERALS == 1) {
            console.log('@@@@@@ OpticalBarrier > status > message ', message)
        }
        return message
    }
