        ...
        if(data.Alarms){
            ...
            for (let key of Object.keys(data.CoinAcceptor)){
                if (key != 'channelsStatus' && key != 'ts'){
                    ...
                    let svg = el.querySelector('svg');
                    if(key == 'status') {
                        if(data.CoinAcceptor[key] == 'offline' || data.CoinAcceptor[key] == 'inactive' || data.CoinAcceptor[key] == 'disabled'){
                            svg.classList.add('text-danger');
                        }else{
                            svg.classList.remove('text-danger');
                        }
            ...
            for (let key of Object.keys(data.BillValidator)){
                if (key != 'channelsStatus' && key != 'ts'){
                    ...
                    let svg = el.querySelector('svg');
                    if(key == 'status') {
                        if(data.BillValidator[key] == 'offline' || data.BillValidator[key] == 'inactive' || data.BillValidator[key] == 'disabled'){
                            svg.classList.add('text-danger');
                        }else{
                            svg.classList.remove('text-danger');
                        }
